...
def _is_cache_valid(cache_dir: Path) -> bool:
    """Check if cached model is valid (exists and not older than 7 days)."""
...
def _resolve_model_path(model_name_or_path: str, model_hub: str = "modelscope") -> str:
    ...
    Returns:
        Path to the downloaded or local model directory.

    Raises:
        ValueError: If unsupported model_hub is specified.
    ...
    if hub == "huggingface":
        ...
        try:
            ...
            try:
                ...
                raise RuntimeError(f"Failed to download model '{model_name_or_path}': {e}") from e
        except Exception as e:
